...
test('kimlik varsa Developer ID + hardened runtime + entitlements', () => {
  ...
  assert.strictEqual(c.identity, 'X (TEAM)', 'ön ek kırpılmalı — electron-builder reddediyor');
...
test('ön eksiz kimlik olduğu gibi kalır', () => {
  const c = macSigningConfig({ APPLE_SIGN_IDENTITY: 'X (TEAM)' }, { warn() {} });
  assert.strictEqual(c.identity, 'X (TEAM)');
});
