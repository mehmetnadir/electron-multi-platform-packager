  ...
  if (BrowserWindow.getAllWindows().length === 0) {
    ...
    // enjeksiyonlarının hiçbiri uygulanmıyordu (Marvel 11 dmg'de doğrulandı).
    {
      ...
      try {
        ...
        // EMPP FS SHIM (2026-08-27): yayıncı uygulaması anahtar/temp dosyalarını CWD'ye göreli
        // fs ile yazıyor (remote modülü yok → exe yolu boş). macOS/Linux'ta CWD "/" ve paket
        // salt-okunur → anahtar kaydedilmiyordu. Renderer'da fs sarılır: yazma userData/work'e,
        // okuma önce work sonra paket. main EMPP_WORK_DIR'i verir. asar AÇIK kalır.
        if (!mainJsContent.includes('EMPP_WORK_DIR')) {
          const readyRegex = /(app\.whenReady\(\))/;
          if (readyRegex.test(mainJsContent)) {
            mainJsContent = mainJsContent.replace(readyRegex,
              "process.env.EMPP_WORK_DIR = require('path').join(app.getPath('userData'), 'work');\n$1");
          } else {
        ...
        try {
          ...
          if (await fs.pathExists(indexPath)) {
            ...
            if (!html.includes('empp-fs-shim.js')) {
              html = html.includes('<head>')
                ? html.replace('<head>', '<head><script src="empp-fs-shim.js"></script>')
          ...
          console.warn('⚠️ fs-shim enjeksiyonu başarısız:', e.message);
        }
